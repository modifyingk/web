...
<body>
	...
	<hr>
	<c:import var="xmldata" url="http://localhost:8888/jsp_study/student.xml" charEncoding="UTF-8"/>
	...
	<c:catch var="ex">
		...
		<x:if select="$xdata//학생[@번호='st0004']">
			번호가 st0004번인 학생 이름 : <x:out select="$xdata//학생[@번호='st0004']/이름"/> <br>
		</x:if>
		<x:choose>
			<x:when select="$xdata//학생[@번호='st0005']">
				번호가 st0005번인 학생 이름 : <x:out select="$xdata//학생[@번호='st0005']/이름"/> <br>
			</x:when>
			<x:otherwise>
